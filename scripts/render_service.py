...
class RenderService:
    ...
    def _apply_photorealistic_materials(self):
        ...
        self.log(f"   ✅ Floor: {material_count['floor']}, Text: {material_count['text']}, Explosions: {material_count['explosions']}")
    ...
    def setup_output(self):
        ...
        self.log(f"   Resolution: {self.scene.render.resolution_x}x{self.scene.render.resolution_y} @ {quality['resolution_percentage']}%")
    ...
    def setup_handlers(self):
        ...
        def on_frame_post(dummy):
            ...
            self.log(f"✅ Frame {current:04d}/{self.scene.frame_end} ({progress:.1f}%){eta_str}")
        ...
        def on_render_complete(dummy):
            if self.start_time:
                ...
                avg_time = total_time / (self.scene.frame_end - self.scene.frame_start + 1)
    ...
    def print_summary(self):
        ...
        self.log(f"Quality:   {self.args.quality} ({QUALITY_PRESETS[self.args.quality]['description']})")
        self.log(f"Materials: {self.args.materials} ({MATERIAL_STYLES[self.args.materials]})")
        self.log(f"Frames:    {self.args.start}-{self.args.end}")
