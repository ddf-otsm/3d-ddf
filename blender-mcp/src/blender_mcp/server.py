...
class BlenderConnection:
    ...
    def send_command(self, command_type: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        # At this point, self.sock is guaranteed to be not None due to the connect() check above
        assert self.sock is not None, "Socket should be connected at this point"
...
def _process_bbox(original_bbox: list[float] | list[int] | None) -> list[int] | None:
    ...
    # Convert all values to integers for the normalized result
    if all(isinstance(i, int) for i in original_bbox):
        # Convert to normalized percentages
        return [int(float(i) / max(original_bbox) * 100) for i in original_bbox]
    else:
        # Handle float values by normalizing them
        return [int(float(i) / max(original_bbox) * 100) for i in original_bbox]
